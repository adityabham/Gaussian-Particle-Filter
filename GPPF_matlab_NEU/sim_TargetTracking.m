...
tvec = [0:Nt-1]*Ts;
figure;
plot(tvec,sum(sqrt((save_pf_x_map - save_x).^2), 2),'-.','LineWidth',2), hold on, grid
plot(tvec,sum(sqrt((save_pf_x_mmse - save_x).^2), 2),'-.','LineWidth',2)
plot(tvec,sum(sqrt((save_gppf_x_map - save_x).^2), 2),'--','LineWidth',2)
plot(tvec,sum(sqrt((save_gppf_x_mmse - save_x).^2), 2),'--','LineWidth',2)
xlabel('time [s]'), ylabel('RMSE [m]')
legend('PF (MAP)', 'PF (MMSE)', 'GPPF (MAP)', 'GPPF (MMSE)')

figure,
plot(tvec,save_pf_Neff,'-.','LineWidth',2), hold on, grid
plot(tvec,save_gppf_Neff,'--','LineWidth',2)
